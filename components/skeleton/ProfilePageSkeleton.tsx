    ...
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-pulse">
            {[1, 2, 3, 4].map((i) => (
                <div
                    key={i}
                    className="p-6 space-y-2 rounded-3xl bg-muted/30 border border-border/40"
                >
                    <div className="h-4 w-24 bg-muted rounded-xl"></div>
                    <div className="h-8 w-16 bg-muted rounded-xl"></div>
                </div>
            ))}
        </div>
    );
...
export function ProfileActivitySkeleton() {
    return (
        <div className="p-6 space-y-6 animate-pulse rounded-3xl bg-muted/30 border border-border/40">
            <div className="h-6 w-40 bg-muted rounded-xl"></div>
        ...
        <div className="space-y-4 animate-pulse">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3].map((i) => (
                    <div
                        key={i}
                        className="h-64 bg-muted/30 rounded-3xl border border-border/40"
                    />
                ))}
            </div>
        </div>
